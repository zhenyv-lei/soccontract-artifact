...
analyze -sva src/verification/core/miter_sodor_c2.sv

elaborate -top miter_sodor_c2 -bbox_mul 256 -bbox_a 1024 -bbox_m plusarg_reader -bbox_m GenericDigitalInIOCell -bbox_m GenericDigitalOutIOCell -bbox_m ClockDividerN -bbox_m EICG_wrapper
clock clk
...
# Check that decoupling can expose different response-valid signals.
cover -name cvr_resp_valid_diff {allow_dmem_gnt_diff &&
    (io_dmem_resp_valid_shared != io_dmem_resp_valid_unc)}

# Check that decoupling can stall only one copy.
...
save -jdb my_jdb_sodor_c2 -capture_setup -capture_session_data -force
